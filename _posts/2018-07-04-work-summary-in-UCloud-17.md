...
- 有引号之类的不必要符号,配置文件内容本质是一种符号标记,没有类型区别是没问题的
- 不能写注释或注释行(不能容忍!) yaml虽然需要注意缩进,但能写注释啊
...
>HTTP隧道(HTTP tunnel)的理解可以对比在TCP/IP上构建PPTP隧道建立VPN链接类似，HTTP隧道技术是在HTTP协议上构建的一个隧道。它的作用是把所有要传输的数据全部封装到HTTP协议里再进行传送。

### 同步、异步、阻塞、非阻塞

Java NIO 是多路复用，属于同步非阻塞

Blocking I/O Non-Blocking I/O asynchronous I/O

- 阻塞：调用是指调用结果返回之前，调用者会进入阻塞状态等待。只有在得到结果之后才会返回(有一段时间的等待)
