...
过去近一年主要的工作任务是IAM系统开发，终于发布上线。

这是一个从0到1的经历过程，本文主要是对这个过程的一个总结。

疫情的春天

在权限控制的设计中，更广为人知的是: **RBAC模型**。
...
IAM就是基于**ABAC模型**。基于属性的访问控制（ABAC）是一种授权策略，基于属性来定义权限。

由于原有系统的RBAC模型已经无法满足实际中较为复杂的权限需求，所以我们决定开始了IAM系统的开发。

我们不了解ABAC和IAM是什么，对我们来说这是新的概念和知识。在对友商和业界标杆AWS IAM等产品进行调研和文档查阅后，对IAM有了一定的认知。但是，显然是无法知道他们的底层设计是如何实现的。
...
在夏天到来之前，我们都在设计服务系统框架和权限系统，并且希望能够兼容现有的权限系统，现有的权限系统需要能够迁移到新的IAM系统。
