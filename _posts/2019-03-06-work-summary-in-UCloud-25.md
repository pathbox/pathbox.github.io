...
### 合并两个账户、用户为一个用户这绝对是一项复杂的工程
 合并两个账户、用户为一个用户这绝对是一项复杂的工程。涉及到的关联表等等会导致复杂度很高，所以，在设计账户系统时，是否能够选择一种归类、分类较好，表与表数据之间关联简单的架构呢
...
### HTTPS最早被国外专业色情网站大量使用,Google也很早就使用

因为专业色情网站需要对传输的数据加密，防止别人盗取

对登入观看者的用户数据有较高的保密性要求
