...
RDB的缺点：这种持久化方式把 Redis 中的全量数据打包成 rdb 文件放在硬盘上。但是执行 RDB 持久化过程的是原进程 fork 出来一个子进程，而 fork 这个系统调用是需要时间的，根据[Redis Lab 6 年前做的实验](https://redis.io/topics/latency#fork-time-in-different-systems)，在一台新型的 AWS EC2 m1.small[^13](linux) 上，fork 一个内存占用 1GB 的 Redis 进程，需要 700+ 毫秒，而这段时间，redis 是无法处理请求的。
