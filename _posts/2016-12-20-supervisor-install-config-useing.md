...
什么情况下我们需要进程管理呢？就是执行一些需要以守护进程方式执行的程序，比如一个后台任务，我使用supervisor 对go项目进行守护进程执行。

除此之外，Supervisor 还能很友好的管理程序在命令行上输出的日志，可以将日志重定向到自定义的日志文件中，能分开创建error错误日志，还能按文件大小对日志进行分割。
...
startsecs=10          ; 在启动10s后无异常，则认为该守护进程服务启动成功
startretries=5        ; 重试次数
stdout_logfile_maxbytes=2GB   ;日志文件的默认大小
user=root                     ;配置了root用户 使用supervisor该守护进程命令
redirect_stderr=true          ; 配置stderr 日志，supervisor会自动帮你新建对应的stderr日志文件，如果有stderr错误会保存在该日志文件中
stdout_logfile=/var/log/supervisor/program_name.log   ; 守护进程的日志输出文件
loglevel=debug                ; 日志等级
